...
class ProductCateController extends CommonController {
    ...
    {
    	$list = M('product_cate')->select();
    	$this->assign('list',$list);
     	$this->display('product/cate_index');
    }
    public function add()
    {
   //  	if(IS_POST){
			// $data['cate_name'] = I('post.cate_name','');
	  //       $res = M('product_cate')->add($data);
	  //       $this->success('新增成功',U('/productcate/index'),1);
	  //       die;
   //  	}
    	$this->display('product/cate_info');
    }
    public function edit($id)
    {	
    	$cate_id = I('id');
    	$res = M('product_cate')->find($id);
    ...
    public function handle()
    {
    	 ...
    	 $data['cate_name'] = I('post.cate_name','');
    	 if($aid>0){
    	 	M('product_cate')->where(array('cate_id'=>$aid))->save($data);
    	 	$this->success('修改成功',U('productcate/index'),1);
    	 }else{
    	 	M('product_cate')->add($data);
    	 	$this->success('添加成功',U('productcate/index'),1);
    	 }
    }
    public function del()
    {
    	$id = I('get.id');
        $ret = M('product_cate')->where(array('cate_id'=>$id))->delete();
        if($ret){
            $this->success('删除成功',U('/productcate/index'),1);
        }else{
            $this->error('删除失败！');
        }
    }
