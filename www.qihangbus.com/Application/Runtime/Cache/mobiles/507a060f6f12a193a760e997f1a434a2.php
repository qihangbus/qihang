...
<div class="iban"><img src="/Public/images/mobiles/ban_mx.jpg" alt="" /></div>
	<div class="mxjilu_t">
		<span class="iconfont icon-dadou"></span>当前可用金豆<b class="color2"><?php echo ($userinfo["student_points"]); ?></b>颗<!-- <a href="<?php echo U('mobile.php/Book/points_record',array('user_id'=>$userinfo['student_id'],'user_flag'=>$user_flag));?>" class="ibtn_1">兑换记录</a> -->
		&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('mobile.php/order/index',array('user_id'=>$userinfo['student_id'],'user_flag'=>$user_flag));?>" class="" style="margin-bottom: 5px;display: inline-block;font-size: 12px;color: #fff;padding: 3px 16px 2px;border-radius: 7px;background-color: #aee078;line-height: 20px;text-align: center;">订单记录</a>

		<a href="<?php echo U('mobile.php/Book/change_log',array('user_id'=>$userinfo['student_id'],'user_flag'=>$user_flag));?>" class="ibtn_1">兑换记录</a>
	</div>
...
<!-- 		  	<ul class="ul_fl clearfix">
  ...
  <ul class="prolist clearfix">
	<?php if(is_array($product)): $i = 0; $__LIST__ = $product;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
		<a href="<?php echo U('mobile.php/Book/book_info',array('product_id'=>$vo['product_id'],'user_id'=>$user_id,'user_flag'=>$user_flag,'user_points'=>$user_points));?>">
		<dl class="dl_pro">
			<dt><img src="<?php echo ($vo["img_url"]); ?>"></dt>
			<dd class="dd_name"><?php echo ($vo["product_name"]); ?></dd>
			<dd class="dd_p">[<?php echo ($vo["content_desc"]); ?>]</dd>
			<dd class="dd_price s_price"><?php echo ($vo["product_price"]); ?>金豆</dd>
			<dd class="dd_price_s" style="display:none;">市场价：100<?php echo ($vo["shop_price"]); ?>元</dd>
		</dl>
		</a>
...
$(function(){
	...
	$("#cate_cf").click(function(){
		...
		var vpoints = "<?php echo ($userinfo["student_points"]); ?>";
		var uflag = "<?php echo ($user_flag); ?>";
		window.location.href="<?php echo U('mobile.php/Book/index');?>&type=2&cateid="+cate_id+"&user_id="+uid+"&user_flag="+uflag+"&student_points="+vpoints;	
	});
