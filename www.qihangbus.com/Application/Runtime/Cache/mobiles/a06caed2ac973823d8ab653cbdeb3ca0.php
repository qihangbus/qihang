	...
	<?php if($address_list != ''): ?><div class="address_t" style="display:none;"><span>编辑</span></div>
		...
		<div class="address_list">
			<?php if(is_array($address_list)): $i = 0; $__LIST__ = $address_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="javascript:void(0);" class="adrdel" aid="<?php echo ($vo["address_id"]); ?>">
				...
				<a href="<?php echo U('mobile.php/MIndex/address_edit/',array('address_id'=>$vo['address_id'],'user_id'=>$user_id,'user_flag'=>$user_flag));?>">	
		...
		<br><br><br>
		<div class="btn_box "><a href="<?php echo U('mobile.php/MIndex/add_address/',array('user_id'=>$user_id,'user_flag'=>$user_flag));?>" class="btn btn2 btn_address">添加新地址</a></div>
	<?php else: ?>
		<dl class="dl_null dl_null_address">
			...
			<dd class="dd_btn"><a href="<?php echo U('mobile.php/MIndex/add_address/',array('user_id'=>$user_id,'user_flag'=>$user_flag));?>" class="btn btn2 btn_address">添加地址</a>  </dd>
		</dl><?php endif; ?>
