...
<div class="wrap2"> 
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="d_time"><?php echo ($key); ?></div>
		<?php if(is_array($vo["lt"])): $i = 0; $__LIST__ = $vo["lt"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><dl onclick="update_message(<?php echo ($v["message_id"]); ?>);" id="mid<?php echo ($v["message_id"]); ?>" class="dd_xx <?php if($v['readed'] < 1): ?>dd_xx_s<?php endif; ?>">
		<dt><?php echo ((isset($v["sender_name"]) && ($v["sender_name"] !== ""))?($v["sender_name"]):'系统消息'); ?> <span><?php echo (date("m-d H:i",$v["sent_time"])); ?></span></dt>
