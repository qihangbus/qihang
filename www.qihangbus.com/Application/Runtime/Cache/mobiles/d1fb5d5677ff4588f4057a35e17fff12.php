...
<div class="wrap2"> 
	...
	<ul class="cart_list order_list">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["order_amount"] > '0'): ?><li class="li_cart" style="margin-bottom:0px;">
			...
			<?php if(is_array($vo["goods_list"])): $i = 0; $__LIST__ = $vo["goods_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gl): $mod = ($i % 2 );++$i;?><dl class="cart_pro" style="border-bottom: 1px solid #f2f2f2;padding:15px 0;height:85px;">
			...
			<label style="display:block;">订单状态 <span class="color1"><?php echo ($vo["order_status_name"]); ?> <?php echo ($vo["shipping_status_name"]); ?> <?php echo ($vo["pay_status_name"]); ?></span><a href="<?php echo U('mobile.php/Order/order_info',array('order_id'=>$vo['order_id']));?>" style="float:right;" class="ibtn_1">订单详情</a></label>
