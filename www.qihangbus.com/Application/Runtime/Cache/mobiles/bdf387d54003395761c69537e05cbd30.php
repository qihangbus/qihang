...
<div class="wrap2">
	<ul class="cart_list ck_list" id="cart_list" style="margin-bottom:50px;">
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="li_cart" data-bid="<?php echo ($vo["book_id"]); ?>" data-cid="<?php echo ($vo["class_id"]); ?>" data-type="<?php echo ($vo["type"]); ?>" style="margin-bottom:0px;padding-top:10px;">
				<dl class="cart_pro">
					...
					<dd class="dd_pname2" style="padding-top:8px;">
						<p>
							...
							<strong>NO.<?php echo ($vo["book_no"]); ?></strong> (<?php echo ($vo["class_name"]); ?> <?php echo ($vo["grade_name"]); ?>)
						</p>
						<p>
...
<a href="<?php echo U('mobile.php/MIndex/index',['id'=>$id]);?>">
			...
			if($('li.li_cart_ck').length == 0){
				...
				layer.open({
					...
					yes:function(index,layero){
						$.post("<?php echo U('receive');?>",{bids:bids,cids:cids,types:types},function(data){
							if(data.status == 1){
