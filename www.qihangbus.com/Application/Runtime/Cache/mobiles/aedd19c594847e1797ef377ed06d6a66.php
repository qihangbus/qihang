...
<div class="wrap2">
	...
	<ul class="cart_list ck_list" id="cart_list" style="margin-bottom:80px;">
		<?php if(empty($returns)): ?><div class="data-empty">
				<p><img src="/Public/images/mobiles/empty.png"><p>
				<p>暂无数据</p>
			</div><?php endif; ?>
		<?php if(is_array($returns)): $i = 0; $__LIST__ = $returns;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["flag"] < '1'): ?><li class="li_cart" cid="<?php echo ($vo["log_id"]); ?>" style="margin-bottom:0px;padding-top:10px;">
					<dl class="cart_pro">
						...
						<dd class="dd_pname2">
							...
							<p style="margin: 5px 0 0 0">
								...
								<?php  $len = mb_strlen($vo['book_name'],'utf-8'); if($len<=6) { echo $vo['book_name']; } else { echo mb_substr($vo['book_name'],0,5,'utf-8')."..."; } ?>
							</p>
							<p><strong>归还时间：</strong><?php echo ($vo["return_time"]); ?></p>
							<a href="<?php echo U('mobile.php/TBorrow/returnRevoke',array('log_id'=>$vo['log_id'],'teacher_id'=>$teacher_id));?>" class="ibtn_1 wh" style="float: right;position: relative;bottom: 45px;">撤销</a>
						</dd>
					</dl>
			...
			if($('li.li_cart_ck').length == 0){
				...
				layer.open({
					...
					yes:function(index,layero){
						$.post("<?php echo U('mobile.php/TBorrow/returnRevoke');?>",{cid:values.join(','),teacher_id:<?php echo ($teacher_id); ?>},function(result){
							if(result.status == 1){
