...
<div class="wrap2"> 
	...
	<ul class="cart_list order_list">
		<?php if(empty($points_list)): ?><div class="data-empty">
			<p><img src="/Public/images/mobiles/empty.png"><p>
			<p>暂无数据</p>
			</div><?php endif; ?>
		<?php if(is_array($points_list)): $i = 0; $__LIST__ = $points_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["order_amount"] > '0'): ?><li class="li_cart" style="margin-bottom:0px;">
			...
			<a href="javascript:void(0);" oid="<?php echo ($vo["order_id"]); ?>" class="ibtn_cancel">取消订单</a>
			<a href="<?php echo U('mobile.php/Order/donetwo',array('order_id'=>$vo['order_id'],'user_id'=>$user_id,'user_flag'=>$user_flag));?>" class="ibtn_1">去结算</a>
			</label>
		...
		var order_id = $(this).attr("oid");
		  layer.open({
    		content: '您确定要取消该订单吗？'
   			,btn: ['确定', '取消']
    		,yes: function(index){
    		$.post("<?php echo U('mobile.php/Order/cancel');?>",{order_id:order_id},function(result){
    			if(result>0){
    				location.reload();
    			}
    		});
