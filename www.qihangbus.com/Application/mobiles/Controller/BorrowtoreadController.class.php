...
class BorrowtoreadController extends CommonController {
    public function index(){
        ...
        if($flag == 1){
            ...
            foreach ($now_info as $k => $v) {
                $access = M('compensate')->where(['student_id'=>$student_id,'book_id'=>$v['book_id'],'compen_status'=>1])->find();
                if($access){
                    $now_info[$k]['compen'] = 1;
                    $url = U('Borrowtoread/index',['student_id'=>$student_id,'flag'=>99,'user_flag'=>3]);
                }
            }
            $url && $this->assign('url',$url);
    ...
    {
        $book_id = I('post.bid',0);
        $user = session('user');
        $parent_id = $user['id'];
        $student = M('students_parent')->where(['parent_id'=>$parent_id])->find();
        $book_no = M('schooltobook')->where(['class_id'=>$student['class_id'],'book_id'=>$book_id])->getField('book_no');
        $price = M('books')->where(['book_id'=>$book_id])->getField('shop_price');
        $data = [
        ...
        $result = M('compensate')->where(['student_id'=>$student['student_id'],'book_id'=>$book_id,'compen_status'=>1])->find();
        if($result){
            $this->success('去支付',U('Borrowtoread/pay',['book_id'=>$book_id,'student_id'=>$student['student_id'],'hf'=>99]));
            exit;
        }else{
            $result = M('compensate')->add($data);
        }
        if($result){
            $this->success('操作成功',U('Borrowtoread/pay',['book_id'=>$book_id,'student_id'=>$student['student_id'],'hf'=>99]));
        }else{
            $this->error('操作失败');
        }
    ...
    public function add_to_borrow()
    {
        ...
        //判断有没有付费
        $student_temp = M("students")->where(['student_id'=>$student_id])->find();
        $paid_num = $student_temp['paid_num'];
        $school_id = $student_temp['school_id'];
        $school_temp = M('schools')->where("school_id = $school_id")->field('semester_one_start,semester_one,semester_two_start,semester_two,try_charge_time')->find();

        if(time() > $school_temp['try_charge_time']){
            if($school_id < 19 && $school_id <> 2) {//旧收费模式
                if (time() > ($school_temp['semester_two_start'] - 86400 * 7) && time() < ($school_temp['semester_two'] + 86400)) {//已到第二学期
                    if ($paid_num <> 2) {//未支付
                        echo 10001;
                        exit();
                    }
                } elseif (time() < ($school_temp['semester_two_start'] - 86400 * 7)) {//第一学期
                ...
                if(time() > $student_temp['paid_expires']){//未付费
                    echo 10001;
                    exit();
                }
            }
        ...
        $num = M('circulation')->where(array('student_id'=>$student_id,'class_id'=>$class_id,'circul_status'=>2))->count(); //已预约条数

        $start_time = date("Y-m-d H:i:s",$starttime);
        $end_time = date("Y-m-d H:i:s",$endtime);
		$num2 = M('circul_log')->where(array('student_id'=>$student_id,'log_time'=>array('between',"$start_time,$end_time")))->count();
        if($num >= $cate_num){
		...
		//判断是否被别人借阅未归还的图书
        $str = M('circulation')->where(array('book_id'=>$data['book_id'],'class_id'=>$class_id,'circul_status'=>1))->find();
        if($str){
            echo 93;
            exit;
        }
    ...
    {
        $circulation_id = I('get.circulation_id',0);
        $result = M('circulation')->where(['circulation_id'=>$circulation_id,'type'=>1])->delete();
        if($result){
            $this->success('操作成功');
        }else{
            $this->error('操作失败');
    ...
    public function pay()
    {
        ...
        if(empty($code)){
            $redirect = urlencode("http://".$_SERVER['HTTP_HOST']."/mobile.php?m=mobile.php&c=Borrowtoread&a=ok&student_id=$student_id&book_id=$book_id&hf=$hf");
            $url = $jsApi->createOauthUrlForCode($redirect,$payment['appid']);
            header("Location:$url");
        }
    }
