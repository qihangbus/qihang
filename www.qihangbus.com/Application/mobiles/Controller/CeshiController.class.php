...
class CeshiController extends CommonController {
    public function index(){
        ...
        if($flag == 1){
            $this->display('Ceshi/index_1');
        }elseif($flag == 2){
            $this->display('Ceshi/index_2');
        }elseif($flag == 99){
            $this->display('Ceshi/index_99');
        }else{
            $this->display('Ceshi/index_1');
        }  
    ...
    public function add_to_borrow()
    {
        ...
        $school_temp = M('schools')->where("school_id = $school_id")->field('semester_one_start,semester_one,semester_two_start,semester_two')->find();
    ...
    public function demage(){
        ...
        $hf = I('hf');
        //添加数据到支付赔偿数据表中
        $stu_info = M('students')->where(array('student_id'=>$student_id))->find();
        $book_info = M('books')->where(array('book_id'=>$book_id))->find();
        M('compensate')->add(array('school_id'=>$stu_info['school_id'],'grade_id'=>$stu_info['grade_id'],
                'class_id'=>$stu_info['class_id'],'student_id'=>$student_id,'add_time'=>time(),'book_id'=>$book_id,
                'book_price'=>$book_info['shop_price'],'compen_type'=>$hf,'compen_status'=>1));
        ...
        if(!isset($_GET['code'])){
            $redirect = urlencode("http://".$_SERVER['HTTP_HOST']."/mobile.php?m=mobile.php&c=Ceshi&a=demage&student_id=$student_id&book_id=$book_id");
            $url = $jsApi->createOauthUrlForCode($redirect,$payment['appid']);
            header("Location:$url");
        }else{
            $code = $_GET['code'];
        ...
        if($openid) {
            ...
            $info['order_sn'] = get_order_sn();
            //$order_amount = '0.01';
            $log_id = M('pay_log')->add(array('order_amount' => $book_info['shop_price'], 'order_type' => 3,
                'user_id' => $student_id, 'user_flag' => 3, 'log_time' => time(), 'remark' => 1));

            $unifiedOrder->setParameter("openid", "$openid");//商品描述
            $unifiedOrder->setParameter("body", '损坏赔偿费用');//商品描述
            $unifiedOrder->setParameter("out_trade_no", "{$info['order_sn']}");//商户订单号
            $unifiedOrder->setParameter("attach", strval($log_id));//商户支付日志
            $unifiedOrder->setParameter("total_fee", strval(intval($book_info['shop_price'] * 100)));//总金额
//            $unifiedOrder->setParameter("total_fee",1);//总金额
            $unifiedOrder->setParameter("notify_url", "http://" . $_SERVER['HTTP_HOST'] . "/wxpay/index.php");//通知地址
            $unifiedOrder->setParameter("trade_type", "JSAPI");//交易类型
            $prepay_id = $unifiedOrder->getPrepayId();
