			...
			<div class="main-content">
				<div class="main-content-inner">
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<form class="form-horizontal" name="admin_group_add" id="admin_group_add" method="post" action="/default.php/Admin/Sys/admin_group_runaccess">
								<input name="id" id="id" type="hidden" value="<?php echo ($admin_group["id"]); ?>" />
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" name="title" id="title" value="<?php echo ($admin_group["title"]); ?>" class="col-xs-10 col-sm-4" readonly/>
										</div>
									</div>
...
3、如果有选择第三级权限，那么一级和二级权限也必须勾选
  ...
  <?php if(is_array($datab)): foreach($datab as $key=>$vo): ?><tr>
	...
	<label>
	<input <?php if(in_array($vo['id'],$admin_rules) == 1): ?>checked<?php endif; ?> name="new_rules[]" class="ace ace-checkbox-2" type="checkbox"  value="<?php echo ($vo["id"]); ?>" /><span class="lbl"> <strong><?php echo ($vo["title"]); ?></strong></span>	</label></td>
  </tr>
  <?php if(is_array($vo['sub'])): foreach($vo['sub'] as $key=>$sub): ?><tr>
	...
	<label>
	<input <?php if(in_array($sub['id'],$admin_rules) == 1): ?>checked<?php endif; ?> name="new_rules[]" id="<?php echo ($sub["id"]); ?>" class="ace ace-checkbox-2" type="checkbox"  value="<?php echo ($sub["id"]); ?>" /><span class="lbl"> <?php echo ($sub["title"]); ?></span></label></td>
  </tr>
  <tr>
    <td height="30" style="padding-left:40px;border-bottom:#E7EBF8 dashed 1px; color:#666666">
	<?php if(is_array($sub['sub'])): foreach($sub['sub'] as $key=>$subb): ?><label class="thopen">
    <input <?php if(in_array($subb['id'],$admin_rules) == 1): ?>checked<?php endif; ?> name="new_rules[]" class="ace ace-checkbox-2" type="checkbox"  id="<?php echo ($subb["id"]); ?>" value="<?php echo ($subb["id"]); ?>" />
    <span class="lbl" style="margin-right:20px;"> <?php echo ($subb["title"]); ?></span>	</label><?php endforeach; endif; ?>	</td>
...
<![endif]-->
		...
		<script src="/Public/2017/assets/js/jquery.form.js"></script>
    		...
    		<!--下拉样式以及JS-->
		<link rel="stylesheet" href="/Public/2017/assets/css/chosen.css" />
		<script src="/Public/2017/assets/js/chosen.jquery.js"></script>
		<script type="text/javascript">
			jQuery(function($) {
				if(!ace.vars['touch']) {
