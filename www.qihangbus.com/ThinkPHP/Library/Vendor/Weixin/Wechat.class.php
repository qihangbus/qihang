...
class Wechat
{
	...
	const MESSAGE_SEND_URL='/message/template/send?';
	...
	public function wx_log($file,$txt)
	{
	   $fp =  fopen($file,'ab+');
...
//			'MsgType'=>self::MSGTYPE_TEXT,
	...
	/**
	 * 发送模板消息
	 * @param array $data 消息结构{"touser":"OPENID","template_id":"ngqIpbwh8bUfcSsECmogfXcV14J0tQlEpBO27izEYtY","url":"http://weixin.qq.com/download",}
	 * @return boolean|array
	 */
	public function sendTemplateMessage($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::MESSAGE_SEND_URL.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	function getUserLocation(){
		if(empty($this->_receive['Latitude'])) return false;
		return array(
