;(function (d) {
    ...
    d.fn.bxSlider = function (p) {
        ...
        var a = {}, c = this, t = d(window).width(), u = d(window).height(), v = function () {
            a.settings = d.extend({}, D, p);
            ...
            a.usingCSS = a.settings.useCSS && "fade" != a.settings.mode && function () {
                    var b = document.createElement("div"), e = ["WebkitPerspective", "MozPerspective", "OPerspective", "msPerspective"], c;
                    for (c in e)if (void 0 !== b.style[e[c]])return a.cssPrefix = e[c].replace("Perspective", "").toLowerCase(), a.animProp = "-" + a.cssPrefix + "-transform", !0;
                    return !1
                }();
            ...
            a.viewport.parent().css({maxWidth: F()});
            ...
            "fade" == a.settings.mode && (a.children.css({
                ...
                zIndex: 0,
                display: "none"
            }), a.children.eq(a.settings.startSlide).css({zIndex: a.settings.slideZIndex, display: "block"}));
            ...
            if (a.settings.infiniteLoop && "fade" != a.settings.mode && !a.settings.ticker) {
                var b = "vertical" == a.settings.mode ? a.settings.minSlides : a.settings.maxSlides, e = a.children.slice(0, b).clone().addClass("bx-clone"), b = a.children.slice(-b).clone().addClass("bx-clone");
            ...
            if ("vertical" == a.settings.mode || a.settings.adaptiveHeight)if (a.carousel) {
                var c = 1 == a.settings.moveSlides ? a.active.index : a.active.index * m(), e = a.children.eq(c);
                for (i = 1; i <= a.settings.maxSlides - 1; i++)e = c + i >= a.children.length ? e.add(a.children.eq(i - 1)) : e.add(a.children.eq(c + i))
            } else e = a.children.eq(a.active.index); else e = a.children;
            ...
            var b = a.settings.slideWidth, e = a.viewport.width();
            ...
            var b = 0;
            if (0 < a.settings.moveSlides)if (a.settings.infiniteLoop)b = Math.ceil(a.children.length / m()); else for (var e = 0, c = 0; e < a.children.length;)++b, e = c + k(), c += a.settings.moveSlides <= k() ? a.settings.moveSlides : k(); else b = Math.ceil(a.children.length / k());
            return b
        }, m = function () {
            ...
            if (a.children.length > a.settings.maxSlides && a.active.last && !a.settings.infiniteLoop)if ("horizontal" == a.settings.mode) {
                var b = a.children.last(), c = b.position();
                f(-(c.left - (a.viewport.width() - b.outerWidth())), "reset", 0)
            } else"vertical" == a.settings.mode && (c = a.children.eq(a.children.length - a.settings.minSlides).position(), f(-c.top, "reset", 0)); else c = a.children.eq(a.active.index * m()).position(), a.active.index == h() - 1 && (a.active.last = !0), void 0 != c && ("horizontal" == a.settings.mode ? f(-c.left, "reset", 0) : "vertical" == a.settings.mode && f(-c.top, "reset", 0))
        }, f = function (b, e, d, g) {
            if (a.usingCSS)b = "vertical" == a.settings.mode ? "translate3d(0, " + b + "px, 0)" : "translate3d(" + b + "px, 0, 0)", c.css("-" + a.cssPrefix + "-transition-duration", d / 1E3 + "s"), "slide" == e ? (c.css(a.animProp, b), c.bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                c.unbind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd");
                s()
            })) : "reset" == e ? c.css(a.animProp, b) : "ticker" == e && (c.css("-" + a.cssPrefix + "-transition-timing-function", "linear"), c.css(a.animProp, b), c.bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                c.unbind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd");
                ...
                var h = {};
                h[a.animProp] = b;
                "slide" == e ? c.animate(h, d, a.settings.easing, function () {
                    s()
                }) : "reset" == e ? c.css(a.animProp, b) : "ticker" == e && c.animate(h, speed, "linear", function () {
                    f(g.resetValue, "reset", 0);
                    l()
                })
            }
        }, x = function () {
            for (var b = "", c = h(), f = 0; f < c; f++) {
                var g = "";
            ...
            f("horizontal" == a.settings.mode ? -b.left : -b.top, "ticker", speed, {resetValue: "horizontal" == a.settings.mode ? -e.left : -e.top})
        }, O = function (b) {
            ...
            var c = Math.abs(b.changedTouches[0].pageX - a.touch.start.x), d = Math.abs(b.changedTouches[0].pageY - a.touch.start.y);
            ...
            if (a.initialized) {
                ...
                if (t != b || u != e)t = b, u = e, c.redrawSlider(), a.settings.onSliderResize.call(c, a.active.index)
            }
        };
        c.goToSlide = function (b, e) {
            if (!a.working && a.active.index != b) {
                ...
                if ("next" == e)a.settings.onSlideNext(a.children.eq(a.active.index), a.oldIndex, a.active.index); else if ("prev" == e)a.settings.onSlidePrev(a.children.eq(a.active.index), a.oldIndex, a.active.index);
                a.active.last = a.active.index >= h() - 1;
                a.settings.pager && r(a.active.index);
                a.settings.controls && A();
                if ("fade" == a.settings.mode)a.settings.adaptiveHeight && a.viewport.height() != n() && a.viewport.animate({height: n()}, a.settings.adaptiveHeightSpeed), a.children.each(function (b, c) {
                    "none" == d(c).css("display") && d(c).fadeOut(a.settings.speed).css({zIndex: 0})
                    ...
                    a.settings.adaptiveHeight && a.viewport.height() != n() && a.viewport.animate({height: n()}, a.settings.adaptiveHeightSpeed);
                    var k = 0, g = {left: 0, top: 0};
                    if (!a.settings.infiniteLoop && a.carousel && a.active.last)if ("horizontal" == a.settings.mode)var l = a.children.eq(a.children.length - 1), g = l.position(), k = a.viewport.width() - l.outerWidth(); else g = a.children.eq(a.children.length - a.settings.minSlides).position(); else a.carousel && a.active.last && "prev" == e ? (g = 1 == a.settings.moveSlides ? a.settings.maxSlides - m() : (h() - 1) * m() - (a.children.length - a.settings.maxSlides), l = c.children(".bx-clone").eq(g), g = l.position()) : "next" == e && 0 == a.active.index ? (g = d(".bx-clone", c).eq(a.settings.maxSlides).position(), a.active.last = !1) : 0 <= b && (g = b * m(), g = a.children.eq(g).position());
